...
class LRUCache(object):
    ...
    def put(self, key, value):
        ...
        if len(self.hashmap) == self.capacity:
            removedKey = self.priority.pop(0)
            self.hashmap.pop(removedKey)
...
if __name__ == '__main__':
    ...
    # cache.get(4);
    cache = LRUCache(10)
    arguments = [[10, 13], [3, 17], [6, 11], [10, 5], [9, 10], [13], [2, 19], [2], [3], [5, 25], [8], [9, 22], [5, 5],
    ...
    for element in range(len(arguments)):
        current = arguments[element]
        if len(current) == 2:
            cache.put(current[0], current[1])
        else:
            cache.get(current[0])